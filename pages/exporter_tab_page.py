...
class ExportTab:
    ...
    def exporter_tab(self):
        # Navigate to the exporter tab
        self.page.locator("//a[@id='nav-contact-tab']").click()
        self.page.wait_for_timeout(1000)
        print("✅ navigated to the exporter page")
    ...
    def Click_Rank_tab(self):
        # Navigate to the Rank tab.
        self.page.locator("span", has_text="Rank").click()

    def verify_rank_table_headers(self):
        self.page.wait_for_timeout(1000)
        # Observe the "Company Name", "Shipment","Shipment Value (USD)" and "Shipper state " columns displayed in proper format..
        expect(self.page.get_by_role("columnheader", name="Shipments")).to_be_visible()
        self.page.wait_for_timeout(1000)
    ...
    def Exporter_apply_filter(self):
        # First row
        ...
        company_locator = first_row.locator("td").nth(2).locator("[data-tip]").first
        company_tip = company_locator.get_attribute("data-tip") if company_locator.count() > 0 else None

        if company_tip and company_tip.strip():
            company_name = company_tip.strip()
        else:
        ...
        sl_no = first_row.locator("td").nth(1).inner_text().strip()
        company_locator_actual = first_row.locator("td").nth(2).locator("[data-tip]").first
        company_tip_actual = company_locator_actual.get_attribute(
            "data-tip") if company_locator_actual.count() > 0 else None

        if company_tip_actual and company_tip_actual.strip():
            company_name_actual = company_tip.strip()
        else:
        ...
        if actual_shipment_value_usd == expected_shipment_value_usd:
            ...
            print(f"valid shipment value usd count:{actual_shipment_value} at {sl_no}")
        # Open the filter panel and navigate to shipper → shipper Jurisdiction Country
        self.page.locator("//span[normalize-space()='Shipper']").click()
        self.page.get_by_text("Shipper Jurisdiction Country").click()
        expect(self.page.locator("#searchIndividualFilterPanel")).to_contain_text(
            "Filter by Shipper Jurisdiction Country")
        #check only one valid jurisdiction country is displayed in the filter panel
        expect(self.page.locator(
            '[class="tw-flex tw-items-center tw-gap-1 tw-font-medium tw-pl-1 tw-text-sm"]')).to_have_count(1,timeout=10000)
        print("Shows one valid jurisdiction country displayed correctly")
        text = self.page.locator(
            '[class="tw-flex tw-items-center tw-gap-1 tw-font-medium tw-pl-1 tw-text-sm"]').first.inner_text().strip()
    ...
    def Enable_second_level_hierarchy(self):
        self.page.pause()
        ...
        expect(
            self.page.get_by_text("Company", exact=False)
        ).to_be_visible(timeout=100000)
    ...
    def second_level_hierarchy_Edit(self):
            ...
            rows = self.page.locator('[class="sc-fqkvVR sc-dcJsrY sc-iGgWBj bIGOvl hMDHCM cZyQSr rdt_TableCell"]')
            row_count = rows.count()
            hscode_values = []
            for i in range(row_count):
                row = rows.nth(i)
                row.wait_for(state="visible", timeout=10000)  # ✅ ensure visible

                hs_code_text = row.inner_text().strip()  # ✅ get text content
                # If you only want the first cell:
            ...
            #validate 4 digit HS code
            self.page.locator('[class="ml-2"]').last.click()
            self.page.get_by_text("6 Digit HS Codes").first.click()
            expect(
                self.page.get_by_text("Company", exact=False)
            ).to_be_visible(timeout=100000)
            rows = self.page.locator('[class="sc-fqkvVR sc-dcJsrY sc-iGgWBj bIGOvl hMDHCM cZyQSr rdt_TableCell"]')
            ...
            for i in range(row_count):
                ...
                hs_code_text = row.inner_text().strip()  # ✅ get text content
                # ✅ Validate that it's only 6 digits
                if re.fullmatch(r"\d{6}", hs_code_text):
                    print(f"✅ Row {i}: HS Code {hs_code_text} is valid (6 digits).")
                else:
                    raise AssertionError(f"❌ Row {i}: Invalid HS Code '{hs_code_text}' (not 6 digits)")
            self.page.wait_for_timeout(3000)
            self.page.locator('[class="ml-2"]').last.click()
            self.page.get_by_text("Countries of Origin").first.click()

    def Disable_second_level_hierarchy(self):
            ...
            expect(self.page.locator("//span[normalize-space()='Countries of Origin']")).not_to_be_visible(timeout=100000)

    def validate_countries_per_company(self, expected_entities: int = 5, max_countries: int = 5):
        # ✅ Select dropdown value
        # self.page.locator("text=Number of second level entities").click()
        # self.page.locator(f"text={expected_entities}").click()

        # ✅ Wait for table update (rows rendered as divs)
        rows = self.page.locator('div.rdt_TableRow')
        expect(rows.first).to_be_visible(timeout=10000)

        # ✅ Locate all company name links
        companies = self.page.locator('[class="trademo-link text-left px-0 text-capitalize "]')
        company_count = companies.count()
        print(f"Found {company_count} companies in list")

        for i in range(company_count):
            company_name = companies.nth(i).inner_text().strip()
            print(f"\n🔎 Checking company: {company_name}")

            # ✅ Find the index of the row containing this company
            row_index = companies.nth(i).locator("xpath=ancestor::*[@class='rdt_TableRow']").evaluate(
                "row => Array.from(row.parentNode.children).indexOf(row)"
            )

            # ✅ Scan subsequent rows until next company link appears
            country_count = 0
            total_rows = rows.count()
            for j in range(row_index + 1, total_rows):
                row = rows.nth(j)

                # if this row has another company link → stop scanning
                if row.locator("a.trademo-link").count() > 0:
                    break

                # else → it's a country row
                row_text = row.inner_text().strip()
                if row_text:  # skip blanks
                    country_count += 1

            print(f"Company: {company_name} → {country_count} countries")
            assert country_count <= max_countries, (
                f"❌ {company_name} has {country_count} countries (expected ≤ {max_countries})"
            )

        print(f"\n✅ All companies have ≤ {max_countries} countries of origin")

    def Download_option(self):
        #Click on Download button
        self.page.get_by_role("button", name="Download").click()
        expect(self.page.get_by_text("Download Unique Exporter")).to_be_visible()
    ...
    def Validate_Colmn_Shown_In_the_Grid_Download_Rank_2nd_level(self):
        import os
        import tempfile
        import openpyxl
        from datetime import datetime
        ...
        EXPECTED_REPORT_TYPE = "Exporters List - 2nd Level"
    ...
    def Analyse_By_Shipment_Value_USD(self):
        ...
        self.page.locator('[class="sc-jlZhew bkvFbV rdt_TableCol_Sortable"]').first.wait_for(state="visible",
                                                                                             timeout=60000)
    ...
    def Filter_by_Shipment_Value(self):
        ...
        expect(self.page.get_by_text("No Unique Exporter Found")).to_be_visible(timeout=100000)
    ...
    def Validate_option_on_selecting_deselect_checkbox(self):
    ...
    def Download_option_Ranktab(self):
        ...
        if self.page.get_by_text("Download Unique Exporter").is_visible():
            expect(self.page.get_by_text("Download Unique Importer")).to_be_visible()
            self.page.locator("//input[@placeholder='To']").clear()
            self.page.locator("//input[@placeholder='To']").fill("1")
            expect(self.page.locator("//span[@class='header-confirm']")).to_contain_text("Confirmation")
        else:
            print("⚠️ 'Download Unique Exporter' not found on page")
            expect(self.page.locator("//span[@class='header-confirm']")).to_contain_text("Confirmation")
    ...
    def Validate_Export_CRM(self):
        ...
        expect(self.page.get_by_role("alert")).to_contain_text(
            "Thanks for showing your interest in the CRM. Your customer success manager will reach out to you at the earliest to discuss more in detail")
    ...
    def Bookmark(self):
        ...
        expect(self.page.get_by_role("alert")).to_contain_text("Companies Bookmarked Successfully")
    ...
    def Verify_Monthly_data(self):
        #Select “Monthly” and validate data updates
    ...
    def Verify_Quaterly_Data(self):
        #Select “Quarterly” and validate data changes accordingly
        self.page.locator('[class="css-19bqh2r"]').nth(1).click()
    ...
    def Verify_Yearly_Data(self):
        #Locate the time range dropdown and validate "yearly" option.
        from datetime import datetime
        self.page.locator('[class="css-19bqh2r"]').nth(1).click()
        self.page.locator('[class=" css-8gqxdx-option"]').nth(0).click()
        expect(self.page.locator("//span[normalize-space()='Company']")).to_be_visible(timeout=50000)
    ...
    def Enable_Trends_second_level_hierarchy(self):
        self.page.pause()
        # Navigate to the “Enable 2nd Level Hierarchy” toggle button
        label = self.page.locator("span.mr-1.two_level_label", has_text="Enable 2nd Level Hierarchy")
        expect(label).to_be_visible(timeout=10000)
        ...
        headers_to_check = [
            ...
            re.compile(r"Countries\s+of\s+Origin", re.I),
            re.compile(r"Total\s+Shipments", re.I),
            re.compile(r"%\s*Overall\s*Change", re.I),
        ]

        for header in headers_to_check:
            locator = self.page.get_by_role("columnheader", name=header)
            expect(locator).to_be_visible(timeout=5000)
            print(f"✅ Header '{header.pattern if hasattr(header, 'pattern') else header}' is visible.")

    def Trends_second_level_hierarchy_Edit(self):
            # In "Country of Origin" column click edit and select any options
            self.page.locator('[class="ml-2"]').last.click()
            expected_editoption = [
                "Trade Routes (Country of Export → Country of Import)",
            ...
            self.page.get_by_text("4 Digit HS Codes").first.click()
            self.page.wait_for_timeout(3000)
            rows = self.page.locator('[class="sc-fqkvVR sc-dcJsrY sc-iGgWBj dQenoR dbbrno gkzfIp rdt_TableCell"]')
            row_count = rows.count()
            for i in range(row_count):
                row = rows.nth(i)
                row.wait_for(state="visible", timeout=10000)  # ✅ ensure visible
    ...
    def Disable_Trends_second_level_hierarchy(self):
            # Toggle OFF the switch
            expect(
                self.page.get_by_text("Company", exact=True)
            ).to_be_visible(timeout=100000)
            self.page.wait_for_timeout(2000)
            self.page.locator('#custom-switch-2').first.wait_for(state="visible", timeout=100000)
            self.page.locator('#custom-switch-2').first.click(force=True, timeout=100000)
            self.page.wait_for_timeout(2000)
            expect(self.page.locator("//span[normalize-space()='Countries of Origin']")).not_to_be_visible(timeout=100000)

    def Validate_Colmn_Shown_In_the_Grid_Download_Trends_2nd_level(self):
        import os
        import tempfile
        import openpyxl
        from datetime import datetime
